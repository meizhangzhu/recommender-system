...
public class Multiplication {
	...
	public static class MultiplicationReducer extends Reducer<Text, Text, Text, DoubleWritable> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context)
			...
			HashMap<String, Double> relation_map = new HashMap<String, Double>();
			HashMap<String, Double> rating_map = new HashMap<String, Double>();

			for (Text value: values) {
				if (value.toString().contains("=")) {
					String[] movie_relation = value.toString().trim().split("=");
					relation_map.put(movie_relation[0], Double.parseDouble(movie_relation[1]));
				}else{
					String[] user_rating = value.toString().trim().split(":");
					rating_map.put(user_rating[0], Double.parseDouble(user_rating[1]));
				}
			}

			for (Map.Entry<String, Double> entry1: relation_map.entrySet()) {
				String movie = entry1.getKey();
				Double relation = entry1.getValue();
				for (Map.Entry<String, Double> entry2: rating_map.entrySet()) {
